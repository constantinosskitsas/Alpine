...
def score(alignment_matrix, true_alignments=None):
    if true_alignments is None:  # assume it's just identity permutation
        ...
        nodes_g2 = [int(true_alignments[node_g1])
                    for node_g1 in true_alignments.keys()]
...
def score_embeddings_matrices(embed1, embed2, topk=None, similarity_threshold=None, normalize=False, true_alignments=None, sim="cosine"):
    similarity_matrix = get_embedding_similarities(
        embed1, embed2, sim_measure=sim)
    alignment_matrix = get_estimated_alignment_matrix(
        similarity_matrix, similarity_threshold, normalize)
    score = score_alignment_matrix(
        alignment_matrix, topk=topk, true_alignments=true_alignments)
    return score
...
def score_alignment_matrix(alignment_matrix, topk=None, topk_score_weighted=False, true_alignments=None):
    ...
    if topk is None:
        ...
        # shouldn't affect much since dividing 0 by anything is 0
        row_sums[row_sums == 0] = 10e-6
        alignment_matrix = alignment_matrix / \
            row_sums[:, np.newaxis]  # normalize

        alignment_score = score(
            alignment_matrix, true_alignments=true_alignments)
    else:
        alignment_score = 0
        ...
        for node_index in range(n_nodes):
            ...
            if target_alignment in node_sorted_indices[-topk:]:
                if topk_score_weighted:
                    alignment_score += 1.0 / \
                        (n_nodes -
                         np.argwhere(sorted_indices[node_index] == target_alignment)[0])
                else:
                    alignment_score += 1
