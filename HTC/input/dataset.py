...
from HTC.input.data_preprocess import DataPreprocess
...
class Dataset:
    ...
    def __init__(self, data_dir):
        ...
        self.output_edge_txt()  #第一次计算orca时需要先导出orca的输入文件格式
        # self.load_edge_features()
        print("Dataset info:")
        print("- Nodes: ", len(self.G.nodes()))
        print("- Edges: ", len(self.G.edges()))

    def output_edge_txt(self):
        edge_np = np.load(self.data_dir + "/edges.edgelist.npy")
        edge_np = np.array(list(filter(lambda x: x[0] != x[1], edge_np))).astype(int)  #去掉自环
        edge = np.insert(edge_np, 0, [edge_np.max() + 1, len(edge_np)], axis=0)
        np.savetxt(self.data_dir + '/orca_in.txt', edge, fmt='%d', delimiter=' ')
    ...
    def load_edge_features(self):
        ...
        if os.path.isfile(feats_path):
            ...
            for idx, matrix in enumerate(edge_feats[0]):
                print(self.edge_features.shape, matrix.shape)
...
def parse_args():
    ...
    parser.add_argument('--groundtruth', default="dataspace/graph/douban/dictionaries/groundtruth")
    parser.add_argument('--output_dir', default="dataspace/graph/douban/statistics/")
