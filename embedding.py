...
def svd_embed(prox_sim, dim):
    #ncv = min(2 * dim + 1, prox_sim.shape[0] - 1)
    #ncv=dim*2
   # if (prox_sim.shape[0]>=ncv):
   #     ncv=prox_sim.shape[0]-2
    #u, s, v = sparse.linalg.svds(prox_sim, dim, return_singular_vectors="u")
    u, s, v = sparse.linalg.svds(prox_sim, dim, return_singular_vectors="u",maxiter=100000,tol=1e-6)
