            ...
            for ptun in range(len(tun)): 
                ...
                for iter in range(iters):
                    ...
                    if(tun[ptun]==1):
                        ...
                        print("k-Size")
                        nm=n_Q
                        #num_nodes = G_Q.number_of_nodes()
                        #num_dummy_nodes = int(0.4 * num_nodes)  # 10% of existing nodes

# Add dummy nodes with new unique indices
                        #new_node_start = max(G_Q.nodes) + 1 if G_Q.nodes else 0  # Get the next available index
                        #G_Q1=G_Q.copy()
                        #G_Q1.add_nodes_from(range(new_node_start, new_node_start + num_dummy_nodes))
                        #n_Q1=G_Q1.number_of_nodes()
                        P, list_of_nodes  = kPMatch( G.copy(),G_Q.copy(),n_Q,t1=3)
                    elif(tun[ptun]==4):
                        print("k-Size")
                        #print("k-Size")
                        nm=n_Q
