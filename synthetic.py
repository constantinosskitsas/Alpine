...
def create_sub_graph_ground_truth(G1: nx.Graph, G2: nx.Graph, n_Q, shuffle='random', method='random walk', p=0.0, gt_G1=None, gt_G2=None):
    ...
    #node_G_Q=node_G
    #if shuffle=='random':
    #    np.random.shuffle(node_G)
    #    np.random.shuffle(node_G_Q)
    #elif shuffle=='fugal':
        #TODO: use fugal/alpine to reduce edit cost
    #    pass
...
for i in range(len(graph_name_and_size) - 1):
    for j in range(i + 1, len(graph_name_and_size)):
        ...
        start1 = time.time()
        n_Q=SG
        for k, trial in product(ksizes, range(10)):
            n_Q=int(k*SG)
