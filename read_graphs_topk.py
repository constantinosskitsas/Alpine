...
def dfs_topK(G, n_Q, n_G1, n_G2): 
    ...
    G1 = list(nx.dfs_preorder_nodes(G, source=source_node))[:0]
    G2=G1
    G3=G1
    while CS==False:
        source_node = random.randint(0, G.number_of_nodes() - 1)
        ...
        # Compute DFS expansions
        G1 = list(nx.dfs_preorder_nodes(G, source=source_node))[:n_Q]
        G2 = list(nx.dfs_preorder_nodes(G, source=source_node))[:n_G1]
        ...
        # Process current node if it's not already in G3 or G2
            if current not in G3 and current not in G2:
                G3.add(current)

            # Push only one neighbor onto the stack to continue DFS in one branch
            for neighbor in G.neighbors(current):
                if neighbor not in G3 and neighbor not in G2:
...
def generate_data(foldernames, n_G, iters, percs):
    for k in range(len(foldernames)):
        ...
        for perc in percs: 
            ...
            for iter in range(iters):
                n_Q1=int(0.1*G.number_of_nodes())+n_Q
                n_Q2=int(0.2*G.number_of_nodes())+n_Q
                dfs_traversal_G,dfs_traversal_G1,dfs_traversal_G2=dfs_topK(G,n_Q,n_Q1,n_Q2)
                random.shuffle(dfs_traversal_G)
                random.shuffle(dfs_traversal_G1)
                random.shuffle(dfs_traversal_G2)
                print(len(dfs_traversal_G),"expected",(n_Q))
                print(len(dfs_traversal_G1),"expected",(n_Q1))
                print(len(dfs_traversal_G2),"expected",(n_Q2))
                #print(n_Q2)
                map_id_to_index = {}
                map_id_to_index1 = {}
                map_id_to_index2 = {}
                for i in range(n_Q): map_id_to_index[dfs_traversal_G[i]] = i
                for i in range(n_Q1): map_id_to_index1[dfs_traversal_G1[i]] = i
                for i in range(n_Q2): map_id_to_index2[dfs_traversal_G2[i]] = i
                folder_ = f'./{folderall}/{foldernames[k]}/TK/{int(perc*100)}/{iter}'
                if not os.path.exists(folder_): os.makedirs(folder_)
                file_subgraph = open(f'{folder_}/subgraph_KG.txt','w')
                for i in range(n_Q):
                ...
                file_nodes = open(f'{folder_}/nodes_KG.txt','w')
                for node in dfs_traversal_G: file_nodes.write(f'{node}\n')
                
                file_subgraph = open(f'{folder_}/subgraph_Q1.txt','w')
                for i in range(n_Q1):
                    for j in range(i, n_Q1):
                        if G.has_edge(dfs_traversal_G1[i],dfs_traversal_G1[j]):
                            file_subgraph.write(f'{map_id_to_index1[dfs_traversal_G1[i]]} {map_id_to_index1[dfs_traversal_G1[j]]}\n')
                file_nodes = open(f'{folder_}/nodes_G1.txt','w')
                for node in dfs_traversal_G1: file_nodes.write(f'{node}\n')
                file_subgraph = open(f'{folder_}/subgraph_Q2.txt','w')
                for i in range(n_Q2):
                    for j in range(i, n_Q2):
                        if G.has_edge(dfs_traversal_G2[i],dfs_traversal_G2[j]):
                            file_subgraph.write(f'{map_id_to_index2[dfs_traversal_G2[i]]} {map_id_to_index2[dfs_traversal_G2[j]]}\n')
                file_nodes = open(f'{folder_}/nodes_G2.txt','w')
                for node in dfs_traversal_G2: file_nodes.write(f'{node}\n')
...
#generate_data_noise(foldernames, n_G, iters, percs,"RW")
generate_data(foldernames, n_G, iters, percs)
