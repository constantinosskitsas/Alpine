...
def compute_full_similarity(model, g, x, edge_types,
                            node_mapping1, node_mapping2,
                            dist='inner'):
    """
    Compute full similarity matrix between all nodes in G1 and all nodes in G2.
    Output is raw similarity (no sigmoid), suitable for Hungarian matching.
    """

    model.eval()
    node_mapping1 = node_mapping1.cpu().numpy()
    node_mapping2 = node_mapping2.cpu().numpy()

    with torch.no_grad():
        out_x = model(g, x, edge_types).cpu().numpy()
        dim = out_x.shape[1]

        weights = model.score_lin.weight[0].cpu().numpy()

        # embeddings for G1 and G2
        x1 = out_x[node_mapping1]      # (#G1, dim)
        x2 = out_x[node_mapping2]      # (#G2, dim)

        # split embeddings
        x1_1, x1_2 = x1[:, :dim//2], x1[:, dim//2:]
        x2_1, x2_2 = x2[:, :dim//2], x2[:, dim//2:]

        # compute raw similarity
        if dist == 'inner':
            S = (weights[0] * x1_1.dot(x2_1.T)
               + weights[1] * x1_1.dot(x2_2.T)
               + weights[2] * x1_2.dot(x2_1.T)
